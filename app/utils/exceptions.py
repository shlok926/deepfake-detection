...
class ForensicsPlatformException(Exception):
    ...
    def log_error(self) -> None:
        ...
        """
        log_payload = {"error_code": self.error_code, "solution": self.solution, "extra_details": self.extra_details}
        logger.error(
            f"[{self.error_code}] {self.description} | Solution: {self.solution}", extra={"extra_info": log_payload}
        )
...
class ModelMissingException(ForensicsPlatformException):
    def __init__(self, model_name: str, expected_path: str):
        super().__init__(
            ...
            solution="Download correct pretrained weight binaries or train the model first using train.py.",
            status_code=404,
