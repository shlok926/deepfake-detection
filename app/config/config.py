...
class AppSettings(BaseSettings):
    ...
    ENV: EnvironmentType = Field(
        default=EnvironmentType.DEVELOPMENT, description="Current environment: development, testing, production"
    )
    ...
    SECRET_KEY: str = Field(
        default="super-secret-key-change-in-production-123456", description="JWT and session signature secret"
    )

    # 2. Database Connection Strings
    DATABASE_URL: str = Field(
        default="sqlite:///./storage/forensics.db", description="SQL Database connection string (PostgreSQL/SQLite)"
    )
    MONGO_URI: Optional[str] = Field(
        default=None, description="MongoDB connection string for metadata/unstructured log cache"
    )
    ...
    ALLOWED_EXTENSIONS: str = Field(
        default="mp4,avi,mov,mkv,mp3,wav", description="Allowed file extensions (comma-separated)"
    )
    ...
    DATASET_CACHE_DIR: str = Field(
        default="storage/dataset_cache", description="Where downloaded benchmark datasets are cached"
    )
    ...
    FUSION_STRATEGY: FusionStrategyType = Field(
        default=FusionStrategyType.LATE_AVERAGE, description="Multimodal fusion strategy"
    )
    ...
    def validate_production_security(self) -> "AppSettings":
        ...
        if not any(self.DATABASE_URL.startswith(p) for p in valid_prefixes):
            raise ValueError(
                f"DATABASE CONFIG ERROR: Invalid URL scheme. Must start with one of: {', '.join(valid_prefixes)}"
            )
