    ...
    def scan_for_duplicates(self, video_paths: List[str]) -> Dict[str, Any]:
        ...
        for i in range(len(path_list)):
            ...
            for j in range(i + 1, len(path_list)):
                ...
                is_meta_match = (
                    sig1["resolution"] == sig2["resolution"]
                    and sig1["duration"] == sig2["duration"]
                    and sig1["fps"] == sig2["fps"]
                    and sig1["file_size_bytes"] == sig2["file_size_bytes"]
                    and sig1["file_size_bytes"] > 0
                )
                ...
                if is_exact or is_near_p or is_meta_match:
                    ...
                    cluster.append({"file_path": p2, "type": reason, "hamming_distance": h_dist})
                    visited.add(p2)

            if cluster:
                duplicates.append({"original_file": p1, "signatures": sig1, "copies": cluster})
                visited.add(p1)
